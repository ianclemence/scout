...
// allModelIDs returns every available model as "provider/id", sorted.
func allModelIDs(core *runtime.Core) []string {
	ms := core.Registry().List(ctxBg(), "")
...
// AvailableModels returns the full registry catalog as a slice, sorted by
// provider then id (registry.List already sorts that way).
