...
Interactive: `/model` opens a searchable picker of models from **configured providers only** with a live fuzzy search, a current/default/provider-sorted list, and Ctrl+S to set the conversation default; an exact model reference (`provider/id`, `provider/id` split, or a unique bare id) switches immediately even for a provider you have not logged into yet. Built-ins remain known offline but are not offered for selection until their provider is configured (or, on a fresh install with nothing configured, the picker shows the full catalog with a warning). Opening the picker refreshes model catalogs in the background (bounded to 15s) and reports the result. `/thinking` sets the reasoning level; `/login` opens the provider selector and then a masked API-key prompt, or `/login <provider>` jumps straight to that provider's key prompt; `/logout` lists stored credentials and removes the one you pick. Keys are encrypted and never displayed. `scout config` shows effective config with secrets redacted. `scout models refresh [provider]` updates the cached model catalog (provider `/models` or Ollama tags; offline falls back to cache, then built-ins).
