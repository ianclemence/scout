...
Interactive: `/model` opens a searchable picker of models from **configured providers only** (Ctrl+S sets the conversation default); an exact `provider/model` argument switches immediately even for a provider you have not logged into yet. Built-ins remain known offline but are not offered for selection until their provider is configured (or, on a fresh install with nothing configured, the picker shows the full catalog with a warning); `/thinking` sets the reasoning level; `/login` opens a staged flow offering both authentication methods — "Sign in with an account" (OAuth/subscription: Anthropic Claude Pro/Max, via a loopback PKCE flow with a paste field for remote browsers) and "Sign in with an API key" (provider → masked key prompt) — or `/login <provider>` jumps straight to that provider's key prompt; `/logout` lists stored credentials and removes the one you pick. Keys and tokens are encrypted and never displayed. Account tokens are refreshed transparently. `scout config` shows effective config with secrets redacted. `scout models refresh [provider]` updates the cached model catalog (provider `/models` or Ollama tags; offline falls back to cache, then built-ins).
